...
namespace PasswordCrackerCentralized
{
    public class Cracking
    {
        ...
        /// </summary>
        public List<UserInfoClearText> RunCracking(WorkInfo workinfo)
        {
            Stopwatch stopwatch = Stopwatch.StartNew();

            List<UserInfoClearText> result = new List<UserInfoClearText>();

            foreach (string word in workinfo.WordList)
            {
                IEnumerable<UserInfoClearText> partialResult = CheckWordWithVariations(word, workinfo.UsersList);
                result.AddRange(partialResult);
            }
            stopwatch.Stop();
            Console.WriteLine(string.Join(", ", result));
            Console.WriteLine("Out of {0} password {1} was found ", workinfo.UsersList.Count, result.Count);
            Console.WriteLine();
